...
async def del_allg(message, gfilters):
    if str(gfilters) not in mydb.list_collection_names() and str(gfilters) not in mydb3.list_collection_names() not in mydb2.list_collection_names():
...
async def count_gfilters(gfilters):
    mycol = mydb[str(gfilters)]
    mycol3 = myd3[str(gfilters)]
    mycol2 = mydb2[str(gfilters)]

    count = mycol.count()
    count = ((mycol.count())+(mycol3.count())+(mycol2.count()))
...
async def gfilter_stats():
    ...
    totalcollections = len(collections)+len(collections3)+len(collections2)
